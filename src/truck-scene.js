...
function initTruckScene() {
  ...
  // Procedural gradient env (sky above, dark ground below)
  const envData = new Float32Array(6 * 3) // 6 faces × RGB
  const envColors = [
    [0.15, 0.22, 0.4],  // +X
    [0.15, 0.22, 0.4],  // -X
    [0.2,  0.3,  0.55], // +Y (sky)
    [0.08, 0.06, 0.04], // -Y (ground)
    [0.18, 0.25, 0.45], // +Z
    [0.1,  0.14, 0.28], // -Z
  ]
  envColors.forEach((c, i) => { envData[i*3]=c[0]; envData[i*3+1]=c[1]; envData[i*3+2]=c[2] })
  const envTex = new THREE.DataTexture(envData, 1, 1, THREE.RGBFormat, THREE.FloatType,
    THREE.CubeReflectionMapping)
  envTex.needsUpdate = true

  const pmrem = new THREE.PMREMGenerator(renderer)
  pmrem.compileCubemapShader()
  const cubeRT = pmrem.fromEquirectangular(new THREE.DataTexture(
    new Float32Array([0.14, 0.2, 0.38, 0.18, 0.26, 0.48, 0.08, 0.06, 0.04, 0.14, 0.2, 0.38]),
    2, 2, THREE.RGBFormat, THREE.FloatType
  ))
